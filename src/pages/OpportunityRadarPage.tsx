...
export function OpportunityRadarPage({ entityType }: { entityType: OpportunityEntityType }) {
  ...
  const headerActions = compactActions ? (
    ...
    <Stack direction="row" spacing={1} useFlexGap sx={{ flexWrap: 'wrap', justifyContent: 'flex-end', maxWidth: 520 }}>
...
function EvaluationDialog({ open, fullScreen, providerStatus, onClose, onEvaluated }: { open: boolean; fullScreen: boolean; providerStatus: RadarProviderStatus | null; onClose: () => void; onEvaluated: (message: string) => void }) {
  const [provider, setProvider] = useState<EvaluationProvider>('Simulated'); const [preview, setPreview] = useState<EvaluationPreview | null>(null); const [busy, setBusy] = useState(false); const [error, setError] = useState('');
  ...
  async function createPreview() { setBusy(true); setError(''); try { setPreview(await previewOpportunityEvaluation(provider)); } catch (err) { setError(getApiErrorMessage(err, 'Unable to preview this evaluation batch.')); } finally { setBusy(false); } }
  async function run() { if (!preview) return; setBusy(true); setError(''); try { const result = await evaluateOpportunities(provider, undefined, preview.confirmationCode); onEvaluated(`${result.evaluatedCount} records evaluated with ${provider === 'Jev' ? 'live Jev' : 'the demonstration evaluator'}.${result.failedCount ? ` ${result.failedCount} provider failures were preserved for review.` : ''}`); } catch (err) { setError(getApiErrorMessage(err, 'Unable to evaluate opportunities.')); } finally { setBusy(false); } }
  return <RadarDialog open={open} fullScreen={fullScreen} title="Evaluate opportunities" busy={busy} onClose={onClose} actions={<><Button onClick={onClose} disabled={busy}>Cancel</Button>{!preview ? <Button variant="contained" onClick={createPreview} disabled={busy}>{busy ? 'Previewing...' : 'Preview batch'}</Button> : <Button variant="contained" color={provider === 'Jev' ? 'warning' : 'primary'} onClick={run} disabled={busy || !preview.allowed}>{busy ? 'Evaluating...' : provider === 'Jev' ? 'Confirm and call Jev' : 'Run demonstration'}</Button>}</>}>
    <Stack spacing={2.5}>{error && <Alert severity="error">{error}</Alert>}<Box><Typography variant="overline" color="primary.main">Step {preview ? 2 : 1} of 2</Typography><Typography variant="h6">{preview ? 'Review the batch' : 'Choose an evaluator'}</Typography></Box>{!preview ? <><Typography variant="body2" color="text.secondary">The batch can include up to 100 records across both lanes.</Typography><Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5 }}>{([['Simulated', 'Demonstration evaluator', 'Deterministic illustrations with no provider usage.'], ['Jev', 'Live Jev', liveAvailable ? 'Live semantic evaluation with usage guardrails.' : 'Not configured on the server.']] as const).map(([value, title, copy]) => <Paper key={value} variant="outlined" component="button" disabled={value === 'Jev' && !liveAvailable} onClick={() => setProvider(value)} sx={{ p: 2, textAlign: 'left', borderRadius: 3, cursor: 'pointer', font: 'inherit', color: 'inherit', bgcolor: provider === value ? 'primary.light' : 'background.paper', borderColor: provider === value ? 'primary.main' : 'divider', '&:disabled': { opacity: 0.5, cursor: 'not-allowed' } }}><Typography sx={{ fontWeight: 800 }}>{title}</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.5 }}>{copy}</Typography></Paper>)}</Box><Alert severity={provider === 'Jev' ? 'warning' : 'info'}>{provider === 'Jev' ? 'Only public or non-confidential descriptions may be sent to TypeSafe. Previewing does not call Jev.' : 'Demonstration results are illustrations, not measured Jev performance.'}</Alert></> : <Paper variant="outlined" sx={{ p: 2.5, bgcolor: SURFACE_SUBTLE, borderRadius: 3 }}><Typography variant="h6">{preview.recordCount} records</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.25 }}>Using {provider === 'Jev' ? 'live Jev' : 'the demonstration evaluator'}</Typography>{provider === 'Jev' && <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', sm: '1fr 1fr' }, gap: 1.5, mt: 2 }}><Metric label="Maximum input" value={`${preview.estimatedMaximumInputTokens.toLocaleString()} tokens`} /><Metric label="Estimated input cost" value={`$${preview.estimatedMaximumCostUsd.toFixed(6)}`} /><Metric label="Batch ceiling" value={`$${preview.maximumBatchCostUsd.toFixed(2)}`} /><Metric label="Rolling usage" value={`${preview.rollingDailyInputTokensUsed.toLocaleString()} / ${preview.rollingDailyInputTokenLimit.toLocaleString()}`} /></Box>}{!preview.allowed && <Alert severity="error" sx={{ mt: 2 }}>{preview.reason ?? 'This batch is blocked by a server guardrail.'}</Alert>}</Paper>}</Stack>
  </RadarDialog>;
}
